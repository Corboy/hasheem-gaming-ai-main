...
const FeaturedBanner = () => {
  const featured = games.find((g) => g.id === "netrunner") || games[0];
          ...
          >
            <Link to={`/game/${featured.id}`} className="block overflow-hidden rounded-xl neon-border">
              <img
                src={featured.image}
                alt={featured.title}
                className="aspect-[3/4] w-full max-w-xs mx-auto rounded-xl object-cover transition-transform duration-500 hover:scale-105"
              />
            </Link>
          </motion.div>
        </div>
